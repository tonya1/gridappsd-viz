...
import { RenderableTopology } from './simulation/topology-renderer';
...
export class AppContainer extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    zip(
      ...
      this.simulationControlService.selectSimulationSnapshotState('renderableTopology')
    )
      .subscribe({
        next: (tuple: [ApplicationState, RenderableTopology]) => {
          this._stateStore.update(tuple[0]);
          this._availableModelDictionaries.set(
            tuple[1].lineName,
            tuple[0].modelDictionary
          );
        }
      });
  }
