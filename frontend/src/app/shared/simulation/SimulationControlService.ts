...
export class SimulationControlService {
  ...
  private constructor() {
    ...
    this._onStateStoreSnapshotStateReceived();
  ...
  private _onStateStoreSnapshotStateReceived() {
    this.selectSimulationSnapshotState('stateStore')
      .pipe(filter(value => value !== null))
      .subscribe({
        next: (stateStore: ApplicationState) => {
          this._modelDictionaryMeasurementMap = new Map();
          for (const measurement of stateStore.modelDictionary.measurements)
            this._modelDictionaryMeasurementMap.set(measurement.mRID, measurement);
        }
      });
  }
