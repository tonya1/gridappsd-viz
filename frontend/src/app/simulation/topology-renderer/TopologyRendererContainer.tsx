...
export class TopologyRendererContainer extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      topology: {
        ...
        edges: [],
        inverted: false,
        lineName: this.activeSimulationConfig.power_system_config.Line_name
      },
      isFetching: true
    };
  ...
  private _updateRenderableTopologyOnSimulationSnapshotReceived() {
    return this._simulationControlService.selectSimulationSnapshotState('renderableTopology')
      .subscribe({
        next: (renderableTopology: RenderableTopology) => {
          this.setState({
            topology: renderableTopology,
            isFetching: false
          });
          for (const node of renderableTopology.nodes) {
            if (node.type === 'switch')
              this._switches.add(node as Switch);
          }
          cache.set(renderableTopology.lineName, renderableTopology);
        }
      });
  }
  ...
  private _useTopologyModelFromCache() {
    const topology = cache.get(this.activeSimulationConfig.power_system_config.Line_name);
    this.setState({
      topology,
      isFetching: false
    });
    this._simulationControlService.syncSimulationSnapshotState({
      renderableTopology: topology
    });
  }
